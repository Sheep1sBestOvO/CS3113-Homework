...
const char  SPRITESHEET_FILEPATH[]  = "Assets/loose sprites.jpg",
            PLATFORM_FILEPATH[]     = "Assets/tileset.jpg",
...
void initialise()
{
    ...
    g_game_state.player->m_walking[g_game_state.player->RIGHT]  = new int[4] { 28, 29, 30, 31 };
    ...
    g_game_state.enemies[0].set_position(glm::vec3(1.5f, -1.5f, 0.0f));
    ...
    g_game_state.enemies[1].set_position(glm::vec3(-2.0f, -3.00f, 0.0f));
...
void update()
{
    ...
    if (g_game_state.player -> get_position().x > 5.0f || g_game_state.player -> get_position().x < -5.0f || g_game_state.player -> m_dead || g_game_state.player -> get_position().y < -3.75f) {
        lose = true;
    }
    for (int i = 0; i < ENEMY_COUNT; i++) {
        if ( g_game_state.enemies[i].m_dead ) {
            win = true;
        }else{
            win = false;
        }
...
void render()
{
    ...
    if (win) {
        DrawText(&g_shader_program, g_text_texture_id, std::string("YOU WIN"), 0.25f, 0.0f, glm::vec3(-1.25f, 0.0f, 0.0f));
    }
    if (lose) {
        DrawText(&g_shader_program, g_text_texture_id, std::string("YOU LOSE"), 0.25f, 0.0f, glm::vec3(-1.25f, 0.0f, 0.0f));
    }
